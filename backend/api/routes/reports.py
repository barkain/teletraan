    ...
    if task.top_sectors:
        for s in task.top_sectors:
            ...
            if m:
                val = float(m.group(1))
                name = s[:m.start()].strip()
                sector_names_list.append(name)
                sector_values_list.append(val)
                sector_colors_list.append("#10B981" if val >= 0 else "#EF4444")
            else:
                sector_names_list.append(s)
                sector_values_list.append(0)
                sector_colors_list.append("#6366F1")
    sector_names_json = json.dumps(sector_names_list)
    sector_values_json = json.dumps(sector_values_list)
...
<div class="container">
  ...
  <div class="charts-row">
    <div class="card">
      <div class="card-label">Confidence Distribution</div>
    ...
    <div class="card">
      ...
      <div class="chart-container" id="chart-sectors"></div>
    </div>
  </div>
...
(function() {{
  ...
  var sectorNames = {sector_names_json};
  var sectorValues = {sector_values_json};
  var sectorColors = {sector_colors_json};
  if (sectorNames.length > 0) {{
    new ApexCharts(document.querySelector('#chart-sectors'), {{
      ...
      theme: darkTheme,
      series: [{{ name: 'Performance', data: sectorValues }}],
      plotOptions: {{
        bar: {{
          horizontal: true,
          borderRadius: 4,
          ...
          colors: {{
            ranges: [
              {{ from: -100, to: -0.001, color: '#EF4444' }},
      ...
      dataLabels: {{
        ...
        style: {{ fontSize: '11px', fontFamily: 'JetBrains Mono, monospace', fontWeight: 600, colors: ['#E2E8F0'] }},
        offsetX: 6
      }},
      xaxis: {{
    ...
    document.querySelector('#chart-sectors').innerHTML =
      '<div style="color:#475569;text-align:center;padding:40px 0;font-size:14px;">No sector data</div>';
  }}
