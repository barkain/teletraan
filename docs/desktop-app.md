...
The easiest way to get Teletraan is to download a pre-built installer from the [Releases page](../../releases).

| Platform | File | Notes |
|----------|------|-------|
| **macOS (Apple Silicon)** | `Teletraan_x.x.x_aarch64.dmg` | Open the `.dmg`, drag Teletraan to Applications |
| **macOS (Intel)** | `Teletraan_x.x.x_x64.dmg` | Open the `.dmg`, drag Teletraan to Applications |
| **Windows** | `Teletraan_x.x.x_x64-setup.msi` | Run the `.msi` installer |

After first launch, configure your LLM provider in **Settings** (or edit `backend/.env` before building from source). See the [LLM Providers](llm-providers.md) guide for details.
...
3. Built binaries are available as workflow artifacts on the run summary page.
...
```
Pushing a `v*` tag triggers the build. The `tauri-apps/tauri-action` automatically creates a **GitHub Release** named `Teletraan v0.1.0` with all platform installers uploaded as release assets.

### Downloading built binaries

**From Releases (recommended for tagged builds):**
1. Go to the [Releases page](../../releases).
2. Find the release matching the version you want.
3. Download the installer for your platform from the **Assets** section.

**From workflow artifacts (manual/branch builds):**
...
7. On tag pushes (`v*`), `tauri-action` creates a GitHub Release with all installers attached as release assets.

## LLM Provider Configuration

The desktop app uses the same LLM provider configuration as the web version. See the [LLM Providers](llm-providers.md) guide for setup instructions. Configure providers via `backend/.env` before building.
